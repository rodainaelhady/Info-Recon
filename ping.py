...
class Ping:
    def __init__(self, master, load_page):
        ...
        back_button = ctk.CTkButton(self.frame, text="Back", height=30,font=("Arail",20),command=lambda: load_page(None))
        back_button.pack(anchor='w', pady=(10, 5))  # زر الرجوع في أقصى اليسار مع مسافة علوية

        # عنوان الصفحة
        label = ctk.CTkLabel(self.frame, text="Ping Page", font=("Arial", 50))
        label.pack(pady=20)  # محاذاة العنوان في المنتصف

        # إطار لإدخال اسم النطاق وزر Ping بجانبه
        ...
        # زر لتنفيذ عملية Ping بجانب حقل الإدخال
        ping_button = ctk.CTkButton(entry_frame, text="Ping", height=50,font=("Arail",30),command=self.perform_ping)
        ping_button.pack(side='left')  # زر Ping بجانب حقل الإدخال
        ...
        copy_button = ctk.CTkButton(self.frame, text="Copy Result",width=450,font=("Arail",25), command=self.copy_result)
    ...
    def perform_ping(self):
        ...
        if domain:
            try:
                # تنفيذ الأمر ping باستخدام subprocess
