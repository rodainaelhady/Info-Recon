...
class Traceroute:
    def __init__(self, master, load_page):
        ...
        back_button = ctk.CTkButton(self.frame, text="Back", height=30, font=("Arial", 20), command=lambda: load_page(None))
        back_button.pack(anchor='w', pady=(10, 5))  # زر الرجوع في أقصى اليسار مع مسافة علوية

        # عنوان الصفحة
        label = ctk.CTkLabel(self.frame, text="Traceroute Page", font=("Arial", 50))
        label.pack(pady=20)  # محاذاة العنوان في المنتصف

        # إطار لإدخال اسم النطاق وزر Traceroute بجانبه
        entry_frame = ctk.CTkFrame(self.frame)
        entry_frame.pack(pady=10)

        # إدخال اسم النطاق
        self.domain_entry = ctk.CTkEntry(entry_frame, placeholder_text="Enter Domain Name", font=("Arial", 20), width=300, height=50)
        self.domain_entry.pack(side='left', padx=(0, 5))  # إدخال الدومين على اليسار

        # زر لتنفيذ عملية Traceroute بجانب حقل الإدخال
        traceroute_button = ctk.CTkButton(entry_frame, text="Traceroute", height=50, font=("Arial", 30), command=self.perform_traceroute)
        traceroute_button.pack(side='left')  # زر Traceroute بجانب حقل الإدخال
        ...
        copy_button = ctk.CTkButton(self.frame, text="Copy Result", width=450, font=("Arial", 25), command=self.copy_result)
    ...
    def perform_traceroute(self):
        ...
        if domain:
            try:
                # تنفيذ الأمر traceroute باستخدام subprocess
